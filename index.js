...
function fenomenEkle(dizi, sırano, profil, takipci, gonderim, platform) {
  const yeniDizi = [...dizi];
  yeniDizi.push({
    number: sırano,
    profile: profil,
...
function platformaGoreCokGonderiYapanFenomen(dizi, platform) {
  ...
  for (let i = 0; i < dizi.length; i++) {
    if (dizi[i].platform == platform) {
      ...
      yeniDizi2.push(dizi[i].posts);
    }
  }
  let enBuyuk = yeniDizi2[0];
  let enBuyukAd;
  for (let i = 0; i < dizi.length; i++) {
    if (yeniDizi2[i] > enBuyuk) {
      enBuyuk = yeniDizi2[i];
    }
    if (yeniDizi2[i] == enBuyuk) {
      enBuyukAd = yeniDizi[i].profile;
    }
  }
  return enBuyukAd;
}
